...
export function Purchase({
  props: { numberOfProperties, propertyType, price, status = 'expired' }
}: {
  props: Purchase
}) {
  const [value, setValue] = useState(numberOfProperties || 1)
  ...
  function onSliderChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const message = {
      ...
      content: `[User has changed to purchase ${newValue} ${propertyType}(s). Total cost: $${(
        newValue * price
      ).toFixed(2)}]`,
  ...
  return (
    <div className="p-4 text-green-400 border rounded-xl bg-zinc-950">
      ...
      <div className="text-lg text-zinc-300">{propertyType}</div>
      ...
      {purchasingUI ? (
        ...
        <>
          ...
          <button
            ...
            onClick={async () => {
              const response = await confirmPurchase(propertyType, price, value)
        ...
        <p className="mb-2 text-white">
          You have successfully purchased {value} {propertyType}(s). Total cost:{' '}
